...
Then setup your origin and upstream to direct to the GitHub repos with your personal account and the organization. The `ssh` version of the secure file transfer will allow you to run shell scripts.

```
cd ~/GitHub
git clone ssh://git@github.com/jaytlennon/InstructorResources
cd InstructorResources
git remote -v
git remote add origin ssh://git@github.com/jaytlennon/InstructorResources
git remote add upstream ssh://git@github.com/QuantitativeBiodiversity/InstructorResources
git remote -v
```
...
You need to be in the directory where the script exists. 
For example, in terminal cd to GitHub/InstructorResources/GitAutomation
The sh. script takes the following arguments: {path to file to copy} 
...
sh GitCopyQB.sh ~/GitHub/QB-2025/2.Worksheets/3.RStudio/ 2.Worksheets/3.RStudio/ participant_repos.txt

# for above, it worked, but I got following error: 
# cp: .//2.Worksheets/3.RStudio: No such file or directory
# (base) lennonj@MacBook-Pro-629 GitAutomation % )
# messing around with forward slashes doens't seem to fix, but the files are copies
...
In this case, we're using wildcards to remove "8.Phylodiversity" from each of 
the student's repo. 
