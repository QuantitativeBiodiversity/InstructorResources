...
An instructor will need to create an upstream repository for each student in the QBstudents GitHub organization (<https://github.com/QBstudents>). In 2017, student repos from previous years were downloaded to local computers and then deleted. 
...
(Jay forked InstructorResources to local machine then copied the the contents of the GitAutomation folder to the QBstudents folder that he created)
In that folder there is a file called `participation_repos.txt`. 
This file should contain all of the names of the students' repositories that you created. 
...
Before you update the student repositories (i.e, add files), you will need to pull files (e.g., Assignments) that were likely merged by you or another instructor. 
Otherwise, you will likely encounter a conflict. 
...
You may need to remove a file that no longer exists in the QB_2019 repo. In this case, you can just cd to the appropriate directory containing student repos, and for example, type the following to remove a directory
